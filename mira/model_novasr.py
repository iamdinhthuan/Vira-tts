"""
MiraTTS with NovaSR upsampler.
Uses NovaSR (50KB, 3600x realtime) instead of FlashSR (1GB, 14x realtime).
"""
...
from mira.codec_novasr import TTSCodecNovaSR


class MiraTTSNovaSR:
    """
    MiraTTS using NovaSR for audio upsampling.
    ~250x faster upsampling, ~20,000x smaller model.
    """
    ...
    def __init__(self, model_dir="YatharthS/MiraTTS", tp=1, enable_prefix_caching=True, cache_max_entry_count=0.2):
        ...
        # Use NovaSR codec instead of original
        self.codec = TTSCodecNovaSR()
